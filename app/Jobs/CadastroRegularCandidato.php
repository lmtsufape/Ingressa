...
class CadastroRegularCandidato implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // try-catch para impedir os failed jobs sofram rollback
        try {
            DB::beginTransaction();
            ...
            foreach ($records as $record) {
                ...
                if (!$candidato) { // Cria um novo candidato e usuário caso ele não exista
                    ...
                    $candidato->user->update(['name' => $record['NO_SOCIAL'] ?? $record['NO_INSCRITO']]);
                }
            ...
            throw $e; // Lança a exceção novamente após o rollback para que o job seja marcado como failed
        }
